...
static struct pin_config ssp0_clk[2] = {
        {
                .name = PN_SSP0_CLK,
                .func = PN_SSP0_CLK,
                .reg.val = 0x2B,
...
static struct pin_config ssp0_cs[2] = {
        {
                .name = PN_SSP0_FS,
                .func = PN_SSP0_FS,
                .reg.val = 0x2B,
...
static int ld9040_power_on_init(struct lcd_info *lcd)
{
	...
	dev_info(&lcd->ld->dev, "%s\n", __func__);
        ...
        pinmux_set_pin_config(&ssp0_clk[0]);
        pinmux_set_pin_config(&ssp0_cs[0]);
	if (!pd) {
...
static int ld9040_power_off(struct lcd_info *lcd)
{
	...
	if (!pd->power_on) {
        ...
        pinmux_set_pin_config(&ssp0_clk[1]);
        pinmux_set_pin_config(&ssp0_cs[1]);
        rc = gpio_request(91, "gpio91");
        if(rc!=0)
                printk("Could not request gpio91\n");
        gpio_direction_output(91,0);
        gpio_set_value(91,0);
        gpio_free(91);
        rc = gpio_request(92, "gpio92");
        if(rc!=0)
                printk("Could not request gpio92\n");
